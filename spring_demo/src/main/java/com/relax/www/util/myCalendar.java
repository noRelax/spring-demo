...
public class myCalendar {
	...
	public static void main(String[] args) {
		...
		int dayOfMon = calendar.get(dayOfMonth);
		int mon = calendar.get(month2) + 1;
		int year3 = calendar.get(year2);

		System.out.println(String.join(":", "一年中第几天", calendar.get(dayOfYear) + "", "一个月中第几天", dayOfMon + "", "月份", mon + "", "年", year3 + ""));
		String week[] = { "星期一", "星期二", "星期三", "星期四", "星期五", "星期六", "星期日" };
		System.out.println("输入的日期是该年的第" + cptDay(year3, mon, dayOfMon) + "天");
		System.out.println("这一天是该年的第" + weekCount(year3, mon, dayOfMon) + "周 " + week[getWeek(year3, mon, dayOfMon)]);
		printCal(year3);
	}
}